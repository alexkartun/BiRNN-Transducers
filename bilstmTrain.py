...
def train(train_data, dev_data, model, type_of_data_set):
    """
    training the model and checking accuracy on dev every 500 sentences, saving the accuracies
    :param type_of_data_set: type of data set (pos/ner)
    :param train_data: training data
    :param dev_data: dev data
    :param model: BiLSTM 2 layer model with specific word embedding representation
    :return: graph of accuracies
    """
    accuracy_graph = {}
    start = time.time()
    for epoch in range(EPOCHS):
        ...
        for index, (sentence, tags) in enumerate(train_data, 1):
            if index % NUMBER_SENTENCES_CHECK_ACCURACY == 0:
                dev_accuracy = compute_accuracy(dev_data, model, type_of_data_set)
                print('dev results = accuracy: {}%'.format(dev_accuracy))
                accuracy_graph[index / 100] = dev_accuracy
...
def compute_accuracy(data, model, type_of_data_set):
    """
    ...
    for sentence, sentence_gold_tags in data:
        ...
        for predicted_tag, gold_tag in zip(sentence_predicted_tags, sentence_gold_tags):
            if type_of_data_set == 'ner' and predicted_tag == 'O' and gold_tag == 'O':
                pass
            elif predicted_tag == gold_tag:
                good += 1
            else:
                bad += 1
...
def save_graph(graph_file_path, accuracy_graph):
    """
    saving the graph of accuracies on dev
    :param graph_file_path: graph's path
    :param accuracy_graph: graph of accuracies
    :return:
    """
    with open(graph_file_path, 'wb') as output:
        pickle.dump(accuracy_graph, output, pickle.HIGHEST_PROTOCOL)
...
def main(argv):
    ...
    graph_file_path = '{}/{}_{}'.format(type_of_data_set, word_representation, 'graph.pkl')
    ...
    print('training...')
    accuracy_graph = train(train_data, dev_data, model, type_of_data_set)
    print('saving the model...')
    save_graph(graph_file_path, accuracy_graph)
